...
void * get_car_id(void * arg)
{
	...
	get_dev_info[1] = 0x02;	//包号= 0 , 命令类型= 0x02
	...
	pcd_config[1] = 0x02;	//包号= 0 , 命令类型= 0x02
	pcd_config[2] = 'B';	//命令= 'B'
	...
	while(1)
	{
		while(1)
		{
			...
			usleep(10*1000); // 延时等待读卡器相应

			bzero(RBuf, 128);   //清空缓冲区
			read(fd, RBuf, 128); // 读取串口中的内容到缓冲区中

			//应答帧状态部分为0 则请求成功
			if(RBuf[2] == 0x00)	 //检查缓冲区中 的 2 位是否为0
			{
				break;
			}
		...
		{
			printf("\r卡号: %#.8x\n", cardid);
            ID = cardid ;  // 把局部变量cardid 赋值给全局变量ID 
